  ...
  const calculateSuggestions = (): Suggestion[] => {
    ...
    if (isAmberZone) {
      ...
      if (goals.meetings_this_month === 0 && goals.days_until_month_end <= 10) {
        suggestions.push({
          ...
          type: 'momentum',
          priority: 3,
          title: 'Invierte 30 min en un Cara a Cara',
      ...
      if (goals.posts_this_week === 0 && goals.comments_this_week === 0) {
        suggestions.push({
          ...
          description: 'Publica en La Calle qué problemas resuelves. Así tus compañeros sabrán a quién mandar cuando surja la ocasión.',
          action: 'Publicar',
          actionRoute: '/feed',
          icon: MessageSquare,
        });
    ...
    if (goals.meetings_this_month === 0 && goals.days_until_month_end <= 10) {
      suggestions.push({
        ...
        description: 'No puedes referir clientes a alguien que no conoces bien. Un Cara a Cara genera la confianza que necesitas para mover negocio.',
        action: 'Agendar reunión',
        actionRoute: '/meetings',
        icon: Calendar,
      });
    ...
    if (goals.posts_this_week === 0 && goals.comments_this_week === 0) {
      suggestions.push({
        ...
        description: 'Publica en La Calle qué problemas resuelves. Cuando tus compañeros entiendan tu negocio, te mandarán clientes.',
        action: 'Publicar',
        actionRoute: '/feed',
        icon: MessageSquare,
      });
